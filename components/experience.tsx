...
export default function Experience({ experiences }: ExperienceProps) {
  ...
  return (
    <section ref={containerRef} id="experience" className="py-32 relative border-t border-white/5">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        <motion.div style={{ opacity, y }}>
          <div className="mb-20">
            <p className="text-xs font-medium tracking-[0.1em] text-white/50 uppercase mb-8 font-dm">Experience</p>
          ...
          <div className="space-y-4">
            {experiences.map((exp, index) => (
              <div 
                key={exp.id} 
                className="group relative flex flex-col md:flex-row gap-6 md:gap-12 py-8 border-t border-white/5 first:border-t-0 hover:bg-white/[0.02] transition-colors p-6 -mx-6 rounded-2xl"
              >
